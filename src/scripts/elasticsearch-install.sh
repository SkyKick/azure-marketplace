...
while getopts :n:m:v:A:R:M:K:S:F:Z:p:a:k:L:C:B:E:H:G:T:W:V:J:N:D:O:P:b:g:xyzldjh optname; do
...
configure_awareness_attributes()
{
  ...
  if [ ! -z "$ZONE"]; then 
      ...
      echo log "[configure_awareness_attributes] did not find zone, setting fault domain and update domain as awareness attribute"
...
configure_elasticsearch_yaml()
{
    ...
    if [[ ${BASIC_SECURITY} -eq 0 ]]; then
      ...
      echo "xpack.security.http.ssl.enabled: false" >> $ES_CONF
    fi
